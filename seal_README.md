...
- **AI Auto-Detection** -- Installer detects your configured cloud provider automatically
...
- **Doctor Mode** -- `./seal_install.command --doctor` for diagnostics without reinstalling
...
- **Friendly Errors** -- Plain-English error messages
...
The installer auto-detects your cloud provider or prompts interactively:

```bash
# macOS
./seal_install.command --ai=foundry      # Azure AI Foundry
./seal_install.command --ai=bedrock      # AWS Bedrock
./seal_install.command --ai=anthropic    # Anthropic API key

# Windows
seal_install.bat --ai=foundry
seal_install.bat --ai=bedrock
seal_install.bat --ai=anthropic
```

If you omit `--ai=`, the installer checks for existing Azure CLI or AWS CLI sessions and offers to use the detected provider. If nothing is detected, an interactive menu appears.

Provider configs live in `config/seal_<provider>.json`. Fill in your endpoints, model names, and SSO URLs.
...
```
claude-code-docker/
  seal_bootstrap.sh             # One-line macOS/Linux installer (curl | bash)
  seal_bootstrap.ps1            # One-line Windows installer (irm | iex)
  seal_setup-claude-mac.command # macOS one-click setup (Rancher Desktop + Docker + install)
  seal_setup-claude.bat         # Windows one-click setup (Rancher Desktop + Docker + install)
  seal_install.command          # macOS/Linux installer (--ai=foundry|bedrock|anthropic|--doctor)
  seal_install.bat              # Windows installer (--ai=foundry|bedrock|anthropic)
  seal_README.md                # This file
  ...
  config/
    seal_foundry.json           # Azure AI Foundry config (fill in your values)
    seal_bedrock.json           # AWS Bedrock config (fill in your values)
    seal_anthropic.json         # Anthropic API config
  welcome/
