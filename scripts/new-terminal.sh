...
if ! tmux has-session -t "$SESSION" 2>/dev/null; then
    exec tmux new-session -s "$SESSION" "CLAUDE_AUTO_LAUNCH=0 bash --init-file $SCRIPTS_DIR/shell-init.sh"
fi

# Create a new window in the existing session and attach to it
tmux new-window -t "$SESSION" -n "$WINDOW_ID" "CLAUDE_AUTO_LAUNCH=0 bash --init-file $SCRIPTS_DIR/shell-init.sh"
