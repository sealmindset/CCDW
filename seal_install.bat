...
REM =============================================================================
REM Claude Code Docker - Public Windows Installer
REM Usage:
REM   seal_install.bat --ai=foundry     Azure AI Foundry (default)
REM   seal_install.bat --ai=bedrock     AWS Bedrock
REM   seal_install.bat --ai=anthropic   Anthropic API key
REM   seal_install.bat                  Interactive prompt or auto-detect
REM =============================================================================
...
echo     2. DOUBLE-CLICK seal_install.bat normally
...
echo     4. Double-click seal_install.bat again
echo.
pause
exit /b 1
...
REM Rancher uses three distros: rancher-desktop, rancher-desktop-data, and Ubuntu.
...
echo [WARN] WSL distributions are corrupted. Repairing automatically...
...
if !ERRORLEVEL! neq 0 (
    ...
    echo   clears stuck ports. Then double-click seal_install.bat again.
    echo.
    pause
    exit /b 1
)
...
echo     5. Double-click seal_install.bat again
echo.
...
echo     3. Double-click seal_install.bat again
echo.
pause
exit /b 1
...
echo     6. Double-click seal_install.bat again
echo.
...
echo        5. Wait until it finishes, then run seal_install.bat again
echo.
...
echo   own account, double-click seal_install.bat again.
echo.
pause
exit /b 0
...
echo   Try running fix-rancher.bat and then seal_install.bat again.
echo.
pause
exit /b 1
...
set "HOST_GITCONFIG=%USERPROFILE%\.gitconfig"
...
if not exist "!KUBE_DIR!" mkdir "!KUBE_DIR!" 2>nul

REM Build host-access args: host networking, kube config, and git identity
set "HOST_ACCESS_ARGS=--add-host host.docker.internal:host-gateway -v "!KUBE_DIR!:/home/coder/.kube""
if exist "!HOST_GITCONFIG!" set "HOST_ACCESS_ARGS=!HOST_ACCESS_ARGS! -v "!HOST_GITCONFIG!:/home/coder/.host-gitconfig:ro""
...
set "CONFIG_FILE=%~dp0config\seal_!AI_PROVIDER!.json"
...
if !ERRORLEVEL! neq 0 (
    ...
    echo ========================================
    echo.
    echo   Fix the issues shown above, then press any key to retry.
    echo.
    pause
    goto :run_preflight
)
...
REM ---------------------------------------------------------------------------
echo.
echo [...]  Configuring AI provider from config\seal_!AI_PROVIDER!.json...
...
echo   (Get this from your team lead or administrator.)
...
if !ERRORLEVEL! neq 0 (
    ...
    if defined REGISTRY_MIRROR (
        ...
        echo       ^(run: az acr import --name !ACR_NAME! --source docker.io/library/node:20-alpine --image node:20-alpine^)
...
echo.
echo   Try: restart Docker, then double-click seal_install.bat again.
echo.
pause
exit /b 1
...
echo --- End of logs ---
echo.
echo   Check the logs above for clues, or try restarting Docker.
echo.
pause
exit /b 1
