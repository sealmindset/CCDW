# Vibe Code Quick Start -- Docker Edition (macOS)

This guide walks you through everything you need to go from zero to building your first app with Claude Code -- using Docker. No Homebrew, no Node.js, no manual tool installs. Just Docker and a browser.
...
> **Why Docker Edition?** The standard macOS setup uses Dockyard to install Node.js, Azure CLI, GitHub CLI, and other tools directly on your Mac. The Docker Edition is an alternative that packages everything into a single container -- you just install Rancher Desktop and double-click a file. Everything else happens in your browser.
...
- [Install Rancher Desktop](#install-rancher-desktop) -- The only thing you install on your Mac
...
This is the **only software you need to install** on your Mac. Everything else runs inside the container.
...
2. Open the `.dmg` file and drag Rancher Desktop to your Applications folder
3. Launch Rancher Desktop from Applications (you'll need Local Admin privileges -- see prerequisite #2)
4. When prompted, select **dockerd (moby)** as the container engine
5. Wait for Rancher Desktop to finish starting up (the icon in your menu bar will stop spinning)

> **Tip:** Rancher Desktop needs to be running whenever you want to use Claude Code Docker. You can set it to launch automatically at login in its preferences.
...
1. Download `install.command` from your team's shared location (or the CCDW repository)
2. **Double-click `install.command`** in Finder

> **First time only:** macOS may show a security warning saying the file is from an unidentified developer. If this happens:
> - Right-click (or Control-click) the file and select **Open**
> - Click **Open** in the dialog that appears
> - This only needs to be done once -- after that, double-clicking works normally
...
1. Open the URL in a new browser tab on your Mac
...
If you see "Can't reach the AI service" but your internet is working, connect to the Sleep Number VPN on your Mac, then try again.

### Container won't start

Make sure Rancher Desktop is running (check for the icon in your menu bar). If it's not running, start it from Applications and wait for it to finish loading, then double-click `install.command` again.

### "install.command" won't open

macOS may block the file the first time. Right-click (or Control-click) the file, select **Open**, then click **Open** in the dialog. This only needs to be done once.

### Need to start fresh

Double-click `install.command` again. It automatically stops the old container and starts a fresh one. Your projects in `~/Documents/GitHub` are preserved.

---

## What's Different from the Standard Setup?

| | Standard Setup (Dockyard) | Docker Edition |
|---|---|---|
| **Install on Mac** | Homebrew, Node.js, Azure CLI, GitHub CLI, Claude Code, /make-it skills | Rancher Desktop only |
| **Setup method** | Terminal + Dockyard script | Double-click `install.command` |
| **Setup time** | 30-60 minutes | 15-20 minutes |
| **Terminal** | macOS Terminal | Web browser |
