...
if ! curl -s --connect-timeout 5 -o /dev/null https://github.com 2>/dev/null; then
    ...
    echo "  Check Wi-Fi and VPN, then try again."
    exit 1
fi
...
if [ -f "$INSTALL_DIR/setup-claude-mac.command" ]; then
...
if [ "$(uname)" = "Darwin" ]; then
    SETUP_SCRIPT="$INSTALL_DIR/setup-claude-mac.command"
else
    SETUP_SCRIPT="$INSTALL_DIR/install.command"
fi
