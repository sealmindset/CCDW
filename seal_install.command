#!/usr/bin/env bash
# =============================================================================
# Claude Code Docker - Public / Generic macOS / Linux Installer
# Usage:
...
#   ./seal_install.command                  Interactive prompt or auto-detect
...
if [ "$RUN_DOCTOR" = "1" ]; then
    ...
    if docker image inspect ghcr.io/sealmindset/claude-code-docker:latest &>/dev/null 2>&1; then
        ...
        echo -e "  ${RED}✗${NC} Image not found — run seal_install.command to download"
    fi
...
if [ "$HAS_PROVIDER" = "1" ] && [ -z "$AI_PROVIDER" ]; then
    ...
    # If no --ai= argument, try auto-detect then prompt interactively
    if [ -z "$AI_PROVIDER" ]; then
        DETECTED_PROVIDER=""

        # Auto-detect: check environment and credentials
        if [ -n "${ANTHROPIC_API_KEY:-}" ]; then
            DETECTED_PROVIDER="anthropic"
            DETECTED_LABEL="Anthropic API (ANTHROPIC_API_KEY found)"
        elif az account show &>/dev/null 2>&1; then
            DETECTED_PROVIDER="foundry"
            DETECTED_LABEL="Azure AI Foundry (az account show succeeded)"
        elif aws sts get-caller-identity &>/dev/null 2>&1; then
            DETECTED_PROVIDER="bedrock"
            DETECTED_LABEL="AWS Bedrock (aws sts get-caller-identity succeeded)"
        fi

        if [ -n "$DETECTED_PROVIDER" ]; then
            echo ""
            echo -e "  ${BLUE}Auto-detected: ${DETECTED_LABEL}${NC}"
            read -p "  Use ${DETECTED_PROVIDER}? [Y/n]: " USE_DETECTED
            case "$USE_DETECTED" in
                [nN]|[nN][oO]) DETECTED_PROVIDER="" ;;
            esac
        fi
    ...
    if [ -n "$AI_PROVIDER" ]; then
        CONFIG_FILE="config/seal_${AI_PROVIDER}.json"
...
echo -e "  To diagnose issues:  ${DIM}./seal_install.command --doctor${NC}"
echo ""
